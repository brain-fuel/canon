...
import Canon.Antlr4.Lex (HookEffect (..), LexerHooks (..), noHooks)
...
antlrLexerHooks = LexerHooks OutsideRule onAction onEmit
  where
    onAction rule _ state = case nameText rule of
      "BEGIN_ARGUMENT"
...
hooksForGrammar :: Grammar ann -> Either (LexerHooks ()) (LexerHooks AdaptorState)
hooksForGrammar grammar =
  case [v | Option (Name "superClass") (OptionValueName (QualifiedName v)) <- grammarOptions grammar] of
    (parts : _) | NonEmpty.last parts == Name "LexerAdaptor" -> Right antlrLexerHooks
    _ -> Left noHooks
